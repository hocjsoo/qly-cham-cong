...
export default function ImageLightbox({ image, onClose }) {
  ...
  const handleDownload = (e) => {
    ...
    a.href = image.url;
    a.download = image.title ? image.title.replace(/[^a-zA-Z0-9_-]/g, "_") + ".jpg" : "image.jpg";
    a.target = "_blank";
  ...
  return createPortal(
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", zIndex: 2, padding: "4px 8px" }}>
        <div style={{ fontSize: "13px", fontWeight: 700, color: "#ffffff", textShadow: "0 1px 4px rgba(0,0,0,0.8)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "50vw" }}>
          {image.title || "Hình ảnh"}
        </div>

        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          <button type="button" onClick={() => changeZoom(zoom - 0.25)} aria-label="Thu nhỏ" title="Thu nhỏ (-)" style={viewerButtonStyle}><ZoomOut size={18} /></button>
          <span style={{ minWidth: "50px", textAlign: "center", fontSize: "12px", fontWeight: 800, color: "#ffffff", fontVariantNumeric: "tabular-nums" }}>{Math.round(zoom * 100)}%</span>
          <button type="button" onClick={() => changeZoom(zoom + 0.25)} aria-label="Phóng to" title="Phóng to (+)" style={viewerButtonStyle}><ZoomIn size={18} /></button>
          <button type="button" onClick={() => rotate(90)} aria-label="Xoay ảnh 90°" title="Xoay ảnh 90° (R)" style={viewerButtonStyle}><RotateCw size={17} /></button>
          <button type="button" onClick={resetView} aria-label="Đặt lại kích thước" title="Đặt lại (0)" style={viewerButtonStyle}><RotateCcw size={17} /></button>
          <button type="button" onClick={handleDownload} aria-label="Tải ảnh về" title="Tải ảnh về máy" style={viewerButtonStyle}><Download size={17} /></button>
          <button type="button" onClick={onClose} aria-label="Đóng" title="Đóng (Esc)" style={{ ...viewerButtonStyle, background: "rgba(239, 68, 68, 0.4)", borderColor: "rgba(239, 68, 68, 0.6)" }}><X size={20} /></button>
        </div>
      </div>
      ...
      <div style={{ minHeight: "28px", paddingTop: "6px", textAlign: "center", color: "rgba(255,255,255,0.7)", fontSize: "11px" }}>
