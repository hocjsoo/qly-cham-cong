...
- **Testing**: Zero-Impact Test Suite (40 Suites / 309 Test Cases in-memory & Supertest)
...
│   ├── tests/                       # 40 Test Suites / 309 Test Cases (Zero-Impact & Supertest)
...
├── TEST_SCENARIOS.md               # Tài liệu 309 kịch bản kiểm thử
└── LICENSE                         # MIT License
```
...
| `/api/users` | userController | ✅ | admin/leader | CRUD nhân viên (3-tier DTO: admin/leader/public) |
...
Chạy bộ test suite 309 kịch bản:
```bash
cd server
npm test
```
Tất cả 309/309 test cases chạy hoàn toàn trên bộ nhớ In-Memory & Supertest, cam kết không tác động đến cơ sở dữ liệu thật trên MongoDB Atlas.
